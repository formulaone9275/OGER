...
with codecs.open(uniprot_kb_file,'r','utf8') as kbf:
    ...
    for line in kbf:
        ...
        if line.startswith('AC   '):
            ...
            for idi in id_list:
                uniprot_kb_dic[idi]+=name_list
        elif line.startswith('DE   Contains:'):
            chain_id_list+=id_list
            
for ki in uniprot_kb_dic.keys():
    if ki in chain_id_list:
        uniprot_kb_chain_dic[ki]=uniprot_kb_dic[ki]

json_file='./Glygen/OGER/uniprot_human_sprot_chain.json'
with open(json_file,'w') as outfile:
    json.dump(uniprot_kb_chain_dic,outfile)